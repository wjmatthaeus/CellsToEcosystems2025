...
ntemp_struct* nt, double naddfrac, int mode) /* mode is MODE_SPINUP or MODE_MODEL */
{
    ...
    // WJM 0323  
    //find the 'ideal livestemc' based on allometry set in epc, this is used for both rmax and recovery by regrowth
	     double new_livestem_leaf_c_ratio = epc->alloc_newstemc_newleafc * epc->alloc_newlivewoodc_newwoodc;
       //below i turn off sapwood allocation when current sapwood c is twice the prescribed allometry
       //these calculations are also made in canopy_et.c, could be streamlined (r_max is the maximum daily m_psi_x)
       double livestem_leaf_c_ratio = cs->livestemc/cs->leafc;
       double r_max = livestem_leaf_c_ratio/new_livestem_leaf_c_ratio;
...
//													printf("gpp = %f ; mresp = %f ; avail_c = %f\n", day_gpp, day_mresp, avail_c);
...
/*normal distr version*/
    double psi = epv->leaf_psi;
	double mu = epc->psi_p_50; /*WJM 0821*/
	double s =  epc->s_psi_p_50;
	//today's potential PRC = 1-PLC, excepting hysteresis
	double PRC = 0.5 * erfc((mu - psi)/(sqrt(2.0)*s));

// WJM 0323 calculate how much recovery of PLC is achieved by the above allocation to livestemc
	// recovery is equal to this allocation expressed as a proportion of the ideal sapwood c
	   //first find ratio of sapwood to leaf c
       //multiply in current leaf c to get ideal sapwood
       double ideal_sapwood_c = cs->leafc * new_livestem_leaf_c_ratio;
       //current allocation as a proportion of this
       double proportion_recovery = cf->cpool_to_livestemc / ideal_sapwood_c;
       
//        if (PRC >= 0.70 && proportion_recovery <= 0.001369863){//&& cs->cpool > 0.0
//               proportion_recovery = 0.001369863;//recover every 2 years //0.002739726;//recover once a year
//              // printf("subsidizing recovery");
//                				//printf("  leafc = %f  "\
//  							, cs->leafc);
//        }
       
       				//printf("  proportion recovery = %f \n "\
 							, proportion_recovery);
 							
       //recover some conductance
       double m_psi_x = epv->m_psi_x; 
       epv->m_psi_x = m_psi_x+proportion_recovery;
...
//        printf("daily allocation:  m_x = %f ;  proportion recovery = %f ---------------------\n "\
// 					, epv->m_psi_x,  proportion_recovery);
// 					
