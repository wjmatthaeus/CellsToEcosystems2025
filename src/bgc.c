...
int bgc(bgcin_struct* bgcin, bgcout_struct* bgcout, int mode)
{
	...
	int dayout;
	double inc_c,n_inc,livestem_leaf_c_ratio;
	double plc, bak_turnover, stem_c_to_recover_plc, bak_alloc_newstemc_newleafc;
	double inc = 0.01;
...
// 	double m_psi_x_yesterday, new_livestem_leaf_c_ratio, max_daily_m_psi_x;//WJM 0323
	...
	bak_turnover = epv.day_livestemc_turnover_increment;//WJM 1021
	...
	for (simyr=0 ; ok && simyr<tmpyears ; simyr++)
	{
		...
		for (yday=0 ; ok && yday<365 ; yday++)
		{
			...
			/*WJM 1021: update epv->day_livestemc_turnover_increment to reflect yesterday's PLC
			'scale turnover to PLC' DURING dry season
			*/
 ...
 */
			...
			///reset things. keep it secret. keep it safe. WJM 1021
// 			if(mode == MODE_MODEL){
				//epv.day_livestemc_turnover_increment == bak_turnover;
// 			}
			...
			canopy radiation interception and transmission, added psn WJM 0921*/
			if (ok && radtrans(&cs, &epc, &metv, &epv, &psn_sun, &psn_shade, sitec.sw_alb))
			{
			...
			/*WJM 1021 incrememtnal embolism multiplier reset
			corresponding to xylem growth
			starting 30 days into wet season, lift the floor of embolsim 
			by 1% per day for 100 days*/

			//start of growing season (currently arbitrary)
// 			if(yday == 150){
// 
// 				//see how much conductivity we lost
// 				plc = 1-epv.m_psi_x;
// 			
// 	////setup carbon required to recover plc from start years carbon
// 	// 			last_stem_c = this_stem_c;
// 	// 			this_stem_c = cs.livestemc+cs.deadstemc-last_stem_c;
// 	// 			stem_c_to_recover_plc = this_stem_c*plc;
// 	// 			n_inc = floor(plc/0.1);
// 	// 			if(n_inc>0.0){
// 	// 			inc_c = stem_c_to_recover_plc/n_inc;
// 	// 			}else{inc_c=0.0;}
// 	// 			
// 				 //alternative method: allocate plc additional carbon to stems for growing season
...
//										,epv->psi, max_daily_m_psi_x, m_psi_x, m_psi_x_soil , m_psi_x_yesterday , m_psi, gl_t_wv_sun, proj_lai);
			...
			if (ok && cs.leafc && metv.dayl)
			{
				...
				if (ok && canopy_et(&metv, &epc, &epv, &wf, &cs, 1))
				{
			...
			/* calculate water summary variables */
			if (ok && nsummary(&ns,&nf,&summary))
			{
				printf("Error in nsummary() from bgc()\n");
				ok=0;
			}

			bgc_printf(BV_DIAG, "%d\t%d\tdone nitrogen summary\n", simyr,yday);
